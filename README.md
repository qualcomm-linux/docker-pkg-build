...
It supports ARM64 package builds across Ubuntu and Debian suites (`noble`, `questing`, `resolute`, `trixie`, `forky`, and `sid`) to ensure consistent and reproducible package builds.
...
ghcr.io/qualcomm-linux/pkg-builder                                             forky                <image id>     About an hour ago   1.50GB
...
- **Supported Suites**: Supports Ubuntu `noble`, `questing`, `resolute` and Debian `trixie`, `forky`, `sid`.
...
- **Automated Workflows**: Integrates with GitHub Actions via the `container-build-and-upload.yml` workflow for CI/CD.
...
Use `--host-tmp-dir` to bind-mount a host directory to container `/tmp`.
The directory is created automatically if it does not exist, and removed after the build completes.
